                        ...
                        <select name="weekStart" onchange="this.form.submit()">
                            ...
                            <%
                                ...
                                for (int i = -4; i <= 4; i++) {
                                    ...
                                    tempCal.add(java.util.Calendar.DAY_OF_MONTH, -daysUntilMonday + (i * 7));
                                    java.util.Date weekStart = tempCal.getTime();
                                    pageContext.setAttribute("weekStart", weekStart);
                                    tempCal.add(java.util.Calendar.DAY_OF_MONTH, 6);
                                    java.util.Date weekEnd = tempCal.getTime();
                                    pageContext.setAttribute("weekEnd", weekEnd);
                            %>
                            <option value="<fmt:formatDate value="${weekStart}" pattern="yyyy-MM-dd"/>"
                                ${param.weekStart == null && i == 0 ? 'selected' : (param.weekStart == '<fmt:formatDate value="${weekStart}" pattern="yyyy-MM-dd"/>' ? 'selected' : '')}>
                                Tuần: <fmt:formatDate value="${weekStart}" pattern="dd/MM"/> - <fmt:formatDate value="${weekEnd}" pattern="dd/MM"/>
                            </option>
                            <%
                                }
                            %>
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                                ...
                                <tr>
                                    ...
                                    <c:forEach begin="0" end="6" var="dayOffset">
                                        ...
                                        <%
                                            ...
                                            tempCal.add(java.util.Calendar.DAY_OF_MONTH, dayOffset);
                                            java.util.Date currentDate = tempCal.getTime();
                                        ...
                                        <c:forEach var="schedule" items="${schedules}">
                                            ...
                                            <c:if test="${schedule.shiftID == shift && scheduleDateStr == currentDateStr}">
                                                ...
                                                <td class="${attendance != null ? (attendance.isPresent ? 'present' : 'absent') : 'notyet'}">
                                                    <div>${schedule.subjectName}</div>
                                                    <div>Phòng ${schedule.roomName}</div>
                                                    <div>(${attendance != null ? (attendance.isPresent ? 'Có mặt' : 'Vắng mặt') : 'Chưa học'})</div>
                                                </td>
                                            </c:if>
                                        </c:forEach>
                                        <c:if test="${!found}">
                        ...
                        <div class="legend">
                            ...
                            <span style="background-color:#ffb3b3;">Vắng mặt</span>
                            <span style="background-color:#ffe5e5;">Chưa học</span>
