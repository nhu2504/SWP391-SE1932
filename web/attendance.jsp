...
<html lang="vi">
    <head>
        ...
        <style>
            ...
            h1.dashboard-title .header-icons a {
                color: #333;
                font-size: 24px;
                text-decoration: none;
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                // Xử lý sự kiện nhấp vào khóa học
                $('.course-info p').click(function () {
                    $('.course-info p').removeClass('selected'); // Xóa lớp selected khỏi tất cả
                    $(this).addClass('selected'); // Thêm lớp selected cho khóa học được chọn
                    const courseId = $(this).data('course-id'); // Lấy courseId từ data attribute

                    // Gửi yêu cầu AJAX để lấy dữ liệu điểm danh
                    $.ajax({
                        url: '${pageContext.request.contextPath}/AttendanceStuServlet',
                        type: 'GET',
                        ...
                        success: function (data) {
                            ...
                            if (data.length === 0) {
                                tableBody.append('<tr><td colspan="6">Không có dữ liệu điểm danh</td></tr>');
                            } else {
                                data.forEach(function (attendance) {
                                    const statusClass = attendance.status === 'Có mặt' ? 'present' :
                                            attendance.status === 'Vắng mặt' ? 'absent' : 'future';
                                    tableBody.append(
                                            `<tr>
                        ...
                        error: function () {
                            $('.attendance-table tbody').html('<tr><td colspan="6">Lỗi khi tải dữ liệu</td></tr>');
                        }
                    });
                });
            });
                                ...
                                <tbody>
                                    ...
                                    <c:forEach var="attendance" items="${attendances}">
                                        <tr>
                                            ...
                                            <td>${attendance.room}</td>
                                            <td>${attendance.teacherName}</td>
                                            <td class="${attendance.status == 'Có mặt' ? 'present' : attendance.status == 'Vắng mặt' ? 'absent' : 'future'}">${attendance.status}</td>
